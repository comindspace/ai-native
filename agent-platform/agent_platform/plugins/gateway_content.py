...
GATEWAY_PLUGIN_VERSION = "0.1.10"


def gateway_description() -> str:
    return "Single GatewayMCP boundary for private MCP servers, company context, and corporate memory."


def _platform_metadata(platform_release: str) -> dict[str, str]:
    return {"platformRelease": platform_release} if platform_release else {}


def gateway_codex_manifest(platform_release: str = "") -> dict[str, Any]:
    return {
        "name": "gateway-mcp",
        ...
        "keywords": ["mcp", "gateway", "agents", "skills"],
        ...
        "configuration": {
            ...
            "GATEWAY_MCP_TOKEN": {
                "type": "secret",
                "description": "Fallback Gateway bearer token for agents without MCP OAuth support.",
                "required": True,
            },
        },
        "metadata": _platform_metadata(platform_release),
        "interface": {
            "displayName": "Gateway MCP",
            "shortDescription": "Connect agents to the GatewayMCP boundary.",
            "longDescription": "Provides a remote MCP entry point for routed private MCP access, company context, and corporate memory. Claude Code authenticates through GatewayMCP OAuth; non-OAuth agents can use GATEWAY_MCP_URL and GATEWAY_MCP_TOKEN as a fallback.",
            "developerName": "coMind",
            "category": "Productivity",
            "capabilities": ["MCP"],
            "defaultPrompt": [
                "Use GatewayMCP for company context, tools, memory, access policy, and audited project work."
            ],
        },
    }


def gateway_readme() -> str:
    return "\n".join(
        [
            "# Gateway MCP Plugin",
            "",
            ...
            "```",
            "",
            f"Authenticate the MCP server when Claude prompts. The endpoint is `{GATEWAY_MCP_PUBLIC_URL}`.",
...
def gateway_hermes_init() -> str:
    return "\n".join(
        [
            '"""Hermes plugin placeholder for remote GatewayMCP integration.',
            "",
            "Configure the remote MCP endpoint with GATEWAY_MCP_URL and",
            "GATEWAY_MCP_TOKEN in the host agent/plugin settings.",
            '"""',
            "",
            "",
            "def register(ctx):",
